...
const cometRedisChannel = 'cometOPT_G'
...
console.log('-----------------------------------------------------------------')
console.log('-----------------------------------------------------------------')
console.log(`${cometRedisChannel} - start - ${timenow()}`)
console.log('-----------------------------------------------------------------')
console.log('-----------------------------------------------------------------')
console.log('-----------------------------------------------------------------')
console.log('-----------------------------------------------------------------')
...
const handleDataChannel = msg => {
  ...
  client_redis.set(cometRedisChannel,ltime)
...
(async () => {
  ...
  publisher.on('message', (channel, message) => {
    ...
    switch (channel) {
      case cometRedisChannel:
        comet.publish(SUBSCRIPTION_CHANNEL,payload,ack => {
          if (!ack.successful) {
            console.log('sub fail',cometRedisChannel,ack)
          } else {
            okNum+=1
          }
        })
  ...
  publisher.subscribe(cometRedisChannel)


  setInterval(() => {
    console.log(cometRedisChannel,{messageNum,okNum,time:timenow()})
  },1000*30)
})();
