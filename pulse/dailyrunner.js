...
(async () =>  {
  const contracts = await query(`SELECT rootsymbol,optioncode,openinterest - prevoi as oich from opt_db where optioncode != '';`)
  ...
  for (var i = 0; i < contracts.rows.length; i++) {
    batch.hmset(contracts.rows[i].optioncode,{
      ...
      underlying:contracts.rows[i].rootsymbol
    })
  }
  batch.exec((err, resp)=> {
    if (err) {
